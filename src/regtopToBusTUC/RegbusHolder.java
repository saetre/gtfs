	...
	public void writePredicates(){
		try {
			...
			for ( Iterator<String> iter = regbus.iterator(); iter.hasNext(); ) {
				...
				//Integer busNumber = Integer.parseInt( iter.next() );	// To remove leading zeroes
				//outFile.write("regbus( "+busNumber+" ).\n");
			}
			outFile.write("\n\n");
			for ( Iterator< String > iter=route.iterator(); iter.hasNext();  ){ //iter.next
